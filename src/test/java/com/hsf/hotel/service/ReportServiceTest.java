...
public class ReportServiceTest {
    ...
    public void testGenerateBookingReport() throws IOException {
        when(bookingRepository.findAllByCheckInDateBetween(any(LocalDate.class), any(LocalDate.class))).thenReturn(Collections.singletonList(new Booking()));
        when(excelExportService.exportBookingsToExcel(any())).thenReturn(new ByteArrayInputStream(new byte[0]));

        ByteArrayInputStream result = reportService.generateBookingReport(LocalDate.now(), LocalDate.now().plusDays(1));
    ...
    public void testGeneratePdfBookingReport() throws IOException {
        ...
        ByteArrayInputStream result = reportService.generatePdfBookingReport(LocalDate.now(), LocalDate.now().plusDays(1));
